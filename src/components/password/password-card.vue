<template>
  ...
  <div :class="`flex items-center gap-4 p-4 text-gray-300 rounded shadow-white1 hover:${ color } hover:text-gray-50`" @click="showPassword()">
    <img class="w-12 h-12 rounded-md" :src="`https://logo.clearbit.com/${ url }?size=60`">
...
export default {
  ...
  methods: {
    ...
    showPassword() {
      this.$emit('showPassword', { url: this.url, name: this.name, user: this.user, color: this.color, secret: this.secret, notes: this.notes })
    }
  },
  watch: {
    lang(newValue) {
